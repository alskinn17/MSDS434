...
func init() {
	...
	//db_connection := "user=postgres dbname=chicago_business_intelligence password=root host=172.17.0.1 sslmode=disable port = 5433"

	//Option 4
	//Database application running on Google Cloud Platform.
	//db_connection := "user=postgres dbname=chicago_business_intelligence password=root host=/cloudsql/final-project-skinner:us-central1:mypostgres sslmode=disable port = 5432"
...
func GetTaxiTrips(db *sql.DB) {
	...
	geocoder.ApiKey = "AIzaSyCT1nrH77wdbMoXaplG7-njlNcY98r5N-M"
...
func GetCovidDetails(db *sql.DB) {
	...
	fmt.Println("Created Table for covid_data")	
	...
	fmt.Println("Covid Data: Received data from SODA REST API for Covid Weekly Data")
	...
	for i := 0; i < len(covid_data_list); i++ {
		...
		sql := `INSERT INTO covid_data ("zip_code" , 
		"week_number" , 
		"week_start" , 
		"week_end" , 
		"cases_weekly",
	...
	fmt.Println("Completed Inserting Rows into the covid_data Table")
...
func GetCCVIDetails(db *sql.DB) {
	...
	fmt.Println("Created Table for ccvi_data")	
	...
	fmt.Println("CCVI Data: Received data from SODA REST API for CCVI")
	...
	for i := 0; i < len(ccvi_data_list); i++ {
		...
		ccvi_category := ccvi_data_list[i].CCVI_score
		
		sql := `INSERT INTO ccvi_data ("geography_type" , 
	...
	fmt.Println("Completed Inserting Rows into the ccvi_data Table")
